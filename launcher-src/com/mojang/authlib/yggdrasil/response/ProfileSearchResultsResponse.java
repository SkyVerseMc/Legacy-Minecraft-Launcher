	...
	public static class Serializer implements JsonDeserializer<ProfileSearchResultsResponse> {
		...
		public ProfileSearchResultsResponse deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
			...
			if (json instanceof JsonObject) {
				...
				if (object.has("error")) result.setError(object.getAsJsonPrimitive("error").getAsString());

				if (object.has("errorMessage")) result.setError(object.getAsJsonPrimitive("errorMessage").getAsString());

				if (object.has("cause")) result.setError(object.getAsJsonPrimitive("cause").getAsString());
